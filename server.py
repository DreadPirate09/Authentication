...
def login():
	...
	checkForMd5Format = re.findall(r"([a-fA-F\d]{32})",dict_data['password']);

	print(checkForMd5Format)

	if len(checkForMd5Format) == 0 :
		return json.dumps({"respons":"wrong password","cause":"the format should be md5 hash"})
